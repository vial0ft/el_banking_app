defmodule ElBankingApp.TransactionApi do
  ...
  @spec new_transaction(list) :: {:error, any} | {:ok, integer, pid}
  def new_transaction(list_of_pids) when length(list_of_pids) > 0 do
    ...
    with {:ok, pid} <- Registry.register(@registry, tr_id, set_of_pids),
         {:ok, tr_id} <- send_all_with_fallback({tr_id, {:tx_init, tr_id}}, set_of_pids) do
      {:ok, tr_id, pid}
    else
      err -> err
    end
  ...
  def add_change(tr_id, pid, action) do
    with {:ok, pids} <- get_pids(tr_id),
         ...
         result <- GenServer.call(pid, {:tx_add, {tr_id, action}}) do
      result
    else
      {:error, _} = err -> err
  ...
  def commit(tr_id) do
    with {:ok, pids} <- get_pids(tr_id),
         {:ok, _} <- send_all_with_fallback({tr_id, {:tx_commit, tr_id}}, pids),
         {:ok, _} <- send_all_with_fallback({tr_id, {:tx_close, tr_id}}, pids) do
      {:ok, tr_id}
    end
  end
  ...
  def close(tr_id) do
    case get_pids(tr_id) do
      {:error, _} = err -> err
      {:ok, pids} -> send_all_with_fallback({tr_id, {:tx_close, tr_id}}, pids)
    end
  end

  @spec reject(any, any) :: {:error, any} | {:ok, {:tx_rollback, any, any}}
    ...
    result =
      case get_pids(tr_id) do
        ...
        {:ok, pids} ->
          ...
          send_rollback({tr_id, reason}, pids)
      end
  ...
  defp send_rollback({tr_id, reason}, []), do: {:ok, {:tx_rollback, tr_id, reason}}

  defp send_rollback(tr_id_reason, [pid | rest]) do
    case GenServer.call(pid, {:tx_rollback, tr_id_reason}) do
      {:error, _} = fallback_error -> fallback_error
      _ -> send_rollback(tr_id_reason, rest)
    end
  end
end
